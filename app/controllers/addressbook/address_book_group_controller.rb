...
module Addressbook
  ...
  def create
    @account_contact_group = AccountContactGroup.new(params[:account_contact_group])
    @account_contact_group.account_id = @current_account.id  
  ...
  def update
    @account_contact_group = AccountContactGroup.where("account_id=? AND id=?",@current_account.id,params[:id]).first
