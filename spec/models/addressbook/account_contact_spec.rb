...
module Addressbook
  ...
  context "Importing vcards" do
    ...
    before do 
      ...
      account_contact_array = double(:contacts)
      @user.stub(:account_contacts).and_return(account_contact_array)
      account_contact_array.stub(:find_by_first_name_and_last_name).and_return(nil)
      account_contact_array.stub(:new).and_return(AccountContact.new)
      @vcard = double(:vcard)
